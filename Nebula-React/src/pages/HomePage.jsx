...
export default function HomePage() {
  ...
  return <><CosmosBackground variant="home" /><TopNav variant="home" /><main className="main">
    ...
    <section className="reveal d2"><header className="section-head"><h3 className="section-title">最近的灵感</h3><Link to="/timeline" className="section-link">查看全部 →</Link></header>
      ...
      <div className="recent-grid">{items.map((item, i) => <article className={`note reveal d${i + 2}`} tabIndex={0} key={item.id} role="button" aria-label={`查看灵感 ${item.title || item.text || ''}`} onClick={() => { setEditId(null); setDetailId(item.id) }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setEditId(null); setDetailId(item.id) } }}><div className="meta"><span className={`pill ${typePill[item.type] || 'idea'}`}>{item.type}</span><span>{item.time}</span></div><p className="excerpt">{item.title ? `${item.title}：` : ''}{item.text || '（无正文）'}</p><div className="footer"><span>{item.attachments?.filter((a) => a.kind === 'image').length ? `🖼 ${item.attachments.filter((a) => a.kind === 'image').length} 张图` : ''}</span><span>{item.attachments?.filter((a) => a.kind === 'audio').length ? `🎙 ${item.attachments.filter((a) => a.kind === 'audio').length} 段音频` : ''}</span><span>{item.sync_status === 'synced' ? '已同步' : '草稿'}</span><button type="button" className="note-edit-btn" aria-label="编辑灵感" onClick={(event) => { event.stopPropagation(); setDetailId(null); setEditId(item.id) }}>编辑</button></div></article>)}</div>
    </section>
